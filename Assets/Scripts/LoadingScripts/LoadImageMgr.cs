...
public class LoadImageMgr
{
    ...
    public IEnumerator LoadImage(string url, Action<Texture2D> loadEnd)
    {
        ...
        string filePath = string.Format("file://{0}/{1}.png", savePath, url.Substring(index+1));
        //from hard disk
        bool hasLoad = false;
    ...
    private void Save2LocalPath(string url, Texture2D texture)
    {
        ...
        {
            int index = url.LastIndexOf('/');
            File.WriteAllBytes(string.Format("{0}/{1}.png", savePath, url.Substring(index + 1)), bytes);
        }
        catch (Exception ex)
        {
...
//#endif
        ...
        //Debug.Log(string.Format("savePath is:{0}", savePath));
        return savePath;
    }
